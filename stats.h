...
public:
    ...
    // The hue (h) value of each pixel is represented by
    // its cartesian coordinates: X = s*cos(h) and Y = s*sin(h).
    // This is done to simplify distance computation. The cumulative sums
    // for X and Y are calculated separately, and then combined
    // when we are doing color difference computation.
	stats(PNG & im);
	...
	//helpers for getAvg
	double calcAvgHueX(pair<int,int> ul, pair<int,int> lr, int pixels);
	double calcAvgHueY(pair<int,int> ul, pair<int,int> lr, int pixels);
	double calcAvgSat(pair<int,int> ul, pair<int,int> lr, int pixels);
	double calcAvgLum(pair<int,int> ul, pair<int,int> lr, int pixels);
  ...
  /* @param ul is (x,y) of the upper left corner of the rectangle
    ...
    * bins holding no pixels are not included in the sum.
		*/
